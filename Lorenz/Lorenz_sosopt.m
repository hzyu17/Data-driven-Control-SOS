...
% yr = 5.0;
% dy = 0.5;
% [y1i, y2i, y3i] = meshgrid(yl:dy:yr, yl:dy:yr, yl:dy:yr);
% yi = [y1i(:), y2i(:), y3i(:)];
% ucont = zeros(size(y1i));
% vcont = zeros(size(y2i));
% wcont = zeros(size(y3i));
% 
% for i1 = 1:numel(y1i)
%     xdot = feval(@(pt) control_dynamics_Lorenz(0, pt, x, cx_sol, ax_sol), yi(i1,: )');
%     ucont(i1)= xdot(1);
%     vcont(i1) = xdot(2);
%     wcont(i1) = xdot(3);
% end
% 
% figure
% quiver(y1i, y2i, y3i, ucont, vcont, wcont);
% xlabel('x_1');
% ylabel('x_2');
% zlabel('x_3');
% axis tight equal;
...
pvar x1 x2 x3
x = [x1; x2; x3];
xinits = [-3,-3,-3; -2,-3,-2; 0,-3,-3; 2,-3, 2; 3,-3,2; -3,3,3; -2,3,3; 0,3,4; 2,3,3; 3,3,2];
...
grid on
tspan = [0 100];
for l1 = 1:size(xinits,1)
    xinit = xinits(l1, :);
    plot3(xinit(1,1),xinit(1,2),xinit(1,3), 'Marker', '*', 'color', 'r')
    hold on
    [tcont{l1}, ycont{l1}] = ode15s(@(t,pt) control_dynamics_Lorenz(t, pt, x, cx_sol, ax_sol), tspan, xinit');
end

hold on; cellfun(@(x) plot3(x(:,1),x(:,2),x(:,3)), ycont); hold off;
