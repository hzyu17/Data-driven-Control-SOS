...
fc_init.LevelList = 0;
text(X_init_cen(1)+0.2, X_init_cen(2)+0.6,'X_0','Color','k','FontSize',10)

X_u_cen_fun = @(x1, x2) r_xu - (x1-X_u_cen(1)).^2 - (x2-X_u_cen(2)).^2;
fc_u = fcontour(X_u_cen_fun, 'r','LineWidth',2);
fc_u.LevelList = 0;
text(X_u_cen(1)-0.2, X_u_cen(2)-0.2, 'X_u','Color','r','FontSize',10)

X_r_cen_fun = @(x1, x2) r_xr - (x1-X_r_cen(1)).^2 - (x2-X_r_cen(2)).^2;
fc_r = fcontour(X_r_cen_fun, 'g','LineWidth',2);
fc_r.LevelList = 0;
text(X_r_cen(1)+0.4, X_r_cen(2)+0.5, 'X_r','Color','g','FontSize',10)

X_d_cen_fun = @(x1, x2) r_Xd - (x1-X_d_cen(1)).^2 - (x2-X_d_cen(2)).^2;
fc_xd = fcontour(X_d_cen_fun, 'g','LineWidth',2);
fc_xd.LevelList = 0;
text(X_d_cen(1)+4.5, X_d_cen(2)+4.5, 'X_d','Color','g','FontSize',10)

X_cen_fun = @(x1, x2) r_X - (x1-X_cen(1)).^2 - (x2-X_cen(2)).^2;
fc_X = fcontour(X_cen_fun, 'g','LineWidth',2);
fc_X.LevelList = 0;
text(X_cen(1)-4.2, X_cen(2)-4.2, 'X','Color','g','FontSize',10)
...
% num_div_gu = diff(cx_sol_sym*rho_safe_sol, x2)*ax_sol_sym - transpose((G.*cx_sol_sym*rho_safe_sol)) * vec_grad(ax_sol_sym, [x1, x2]);
% div_all = div_pf_sym * ax_sol_sym^2 + num_div_gu;
...
fc_init = fcontour(X_init_cen_fun, 'k','LineWidth',2);
fc_init.LevelList = 0;
text(X_init_cen(1)+0.1, X_init_cen(2)+0.1,'X_0','Color','k','FontSize',10)

fc_u = fcontour(X_u_cen_fun, 'r','LineWidth',2);
fc_u.LevelList = 0;
text(X_u_cen(1)+0.1, X_u_cen(2)+0.1, 'X_u','Color','r','FontSize',10)

fc_r = fcontour(X_r_cen_fun, 'g','LineWidth',2);
fc_r.LevelList = 0;
text(X_r_cen(1)+0.1, X_r_cen(2)+0.1, 'X_r','Color','g','FontSize',10)
