function [data_file_name] = ocp_safety_rational_main(lambda, eps, geo)
...
for i_c = 1:dim_m
    cx_sol_sym(i_c) = p2s(clean_polynomial(s2p(cx_sol_sym(i_c)), 1e-6));
    cx_sol = [cx_sol; s2p(cx_sol_sym(i_c))];
end
ax_sol_sym = sosgetsol(sos_prog, poly_a_sym);
ax_sol_sym = p2s(clean_polynomial(s2p(ax_sol_sym), 1e-6));
ax_sol = s2p(ax_sol_sym);
wx_sol_sym = sosgetsol(sos_prog, poly_w_sym);
wx_sol_sym = p2s(clean_polynomial(s2p(wx_sol_sym), 1e-6));
wx_sol = s2p(wx_sol_sym);
bx_sol_sym = sosgetsol(sos_prog, poly_b_sym);
bx_sol = s2p(bx_sol_sym);

ax_sol_sym = vpa(ax_sol_sym, 4)
bx_sol_sym = vpa(bx_sol_sym, 4);
for i_c = 1:dim_m
    cx_sol_sym(i_c) = vpa(cx_sol_sym(i_c), 4)
end
wx_sol_sym = vpa(wx_sol_sym, 4);

%% safety verification

% constraints for safety
% 1. rho > 0 in X_init
ineq_safe_1 = poly_safe_rho_sym - poly_safe_s1 * poly_X_init;
...
num_div_gu = vec_div(G*poly_safe_rho_sym*cx_sol_sym, [x1;x2])*ax_sol_sym - transpose((G*cx_sol_sym*poly_safe_rho_sym)) * vec_grad(ax_sol_sym, [x1, x2]);
...
ineq_safe_2 = div_pf_sym * ax_sol_sym^2 + num_div_gu - poly_safe_s2 * poly_X + poly_safe_s3 * poly_X_r  ;
ineq_safe_2 = vpa(ineq_safe_2, 8);
sos_safety_prog = sosineq(sos_safety_prog, ineq_safe_2);

% 3. rho < 0 in X_u + (X_d - X)
ineq_safe_3 = - poly_safe_rho_sym - poly_safe_s6*poly_X_u;
ineq_safe_3 = vpa(ineq_safe_3, 8);
sos_safety_prog = sosineq(sos_safety_prog, ineq_safe_3);

ineq_safe_4 = - poly_safe_rho_sym - poly_safe_s4*poly_X_d + poly_safe_s5*poly_X;
...
data_file_name = ['experiments/',date,'_results_SOS_Rational_lbd_',num2str(lambda),'_eps_',num2str(eps),'.mat'];
disp(data_file_name)
save(data_file_name)
